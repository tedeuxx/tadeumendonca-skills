...
The third observability pillar (with `/logging` + `/metrics`): **AWS Lambda Powertools Tracer** over **X-Ray** — see a request across the BFF and its downstream calls (DynamoDB, Redis, SES, future microservices).
...
```
- Enable **active tracing** on the Lambda (`tracing_mode = "Active"` — `/lambda`) and the API GW stage; sampling rules + service map are the service side (`/cloudwatch-xray`).
- The framework wiring (a middleware that opens a segment per request, marks cold start + response, closes on error) lives in `/framework-hono`.
