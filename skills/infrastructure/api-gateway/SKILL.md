...
- **WAF** rate-based rules (per-IP) front the stage via the REGIONAL WebACL (`/waf`) — the per-IP guard the HTTP API couldn't have.
...
## Conventions
- Cert via `/acm` (regional cert in us-east-1); custom-domain naming via `/route53` (alias → `aws_api_gateway_domain_name.regional_domain_name` / `regional_zone_id`); ids to SSM (`gateway-id` = REST API id, `gateway-url`) via `/ssm`. Contract generation: `/openapi`.
- Raw `aws_api_gateway_*` is justified glue (no official module fits the OpenAPI-body + reimport flow) — `/terraform`.
## Pros & cons
**Pros**
