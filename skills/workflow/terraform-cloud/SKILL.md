---
description: Configure Terraform Cloud as the remote state backend — a workspace per environment, Local execution mode, and the pipeline-only apply and destroy policy that leaves a laptop read-only. Use when adding a workspace, pointing CI at TFC state, or explaining why an apply must never run locally. Not for the Terraform configuration itself (see terraform).
---
...
- See `/terraform` (overall usage) and `/github-actions` (the runner).
