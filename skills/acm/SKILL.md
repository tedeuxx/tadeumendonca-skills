...
The environment is the subdomain boundary (`/route53`), so there is **one ACM certificate per environment**, each a wildcard over that environment's subdomain plus the environment's own host:
...
- Consumed by `/cloudfront`, `/api-gateway`, `/cognito`.
## Pros & cons
**Pros**
