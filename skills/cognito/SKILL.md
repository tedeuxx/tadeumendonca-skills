...
Module: **`lgallard/cognito-user-pool/aws`** (auth.tf) — there is **no** official `terraform-aws-modules` Cognito module, so we use lgallard, the established community one (`/terraform` module policy). Cognito issues the JWT the API GW authorizer validates; the SPA holds it via the Cognito SDK / Amplify (`/authentication`).
...
Federated users aren't auto-grouped, so a small Lambda does it (`/lambda`):
...
- Exec role: `cognito-idp:AdminAddUserToGroup` + `AdminListGroupsForUser` scoped to the pool ARN (`/iam`).
...
The **SPA runs the Authorization Code + PKCE flow via the Cognito SDK / Amplify** and holds/refreshes the JWT; it sends `Authorization: Bearer` to the API GW, whose **Cognito JWT authorizer** validates per route (`/api-gateway`). The **BFF has no auth code** — it only reads claims. No client secret exists (public client).
...
## Conventions
- **Profiles:** public (no auth, no group) / `registered` (any Google sign-in) / `admin` (email allowlist). REGIONAL WAF fronts the hosted UI (`/waf`).
