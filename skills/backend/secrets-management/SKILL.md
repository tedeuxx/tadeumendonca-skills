...
- Naming: `<project>/{env}/{component}` (e.g. `<project>/staging/redis`). The data tier needs no secret — DynamoDB is IAM-only (`/dynamodb`). See `/elasticache`.

## Decision & trade-off
- **Pattern B for secrets: IaC owns the config (the ARN in env/SSM); code reads the value at runtime.** The secret value never sits in the env, the bundle, tfvars, or state — only the non-sensitive ARN travels there. Cross-ref the Lambda Pattern-B split (`/lambda`). *Trade-off:* a runtime fetch (one cold-start round trip to Secrets Manager) instead of baking the value into an env var.
