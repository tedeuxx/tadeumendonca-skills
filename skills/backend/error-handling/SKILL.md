---
description: Throw typed errors in server handler code — AppError, NotFoundError and Unauthorized rather than returning a 4xx inline — with one response shape for every failure. Use when a route needs to fail, standardising an error payload across handlers, or reviewing code that builds status codes by hand. Not for recording the failure (see logging).
---

Implement or review HTTP error handling in the BFF.

Context: $ARGUMENTS

Conceptual skill — the error model and response shape. The framework-specific central handler (how thrown errors map to responses) is wired in `/framework-hono`.
...
- `500` never leaks internals (logged via Powertools — `/logging`).
- Schema-validation failures map to a `400` `ValidationError` so they share the shape.
- The central handler that catches thrown errors and writes the response is wired in `/framework-hono`.
