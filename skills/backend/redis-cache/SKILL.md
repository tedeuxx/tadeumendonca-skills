...
- AUTH token from Secrets Manager (`/secrets-management`); endpoint from `REDIS_ENDPOINT` (IaC). Provisioned in `/elasticache`.
- Emits `cache_hits_total` / `cache_misses_total` — see `/metrics`.
