...
- **API Gateway:** a **Cognito JWT authorizer** validates the token on every request before it reaches the BFF (`/api-gateway`).
...
4. BFF handler reads claims (RBAC/shaping — `/action-types`), calls the domain logic / microservices, **aggregates** into a screen-shaped payload, returns it.
...
- **Async:** **SNS** pub/sub for fire-and-forget (e.g. notifications) — the BFF publishes a domain event, a subscribed Lambda consumes (+ DLQ). See `/sns`, `/notifications`.
- The **BFF owns the public contract** (its OpenAPI at root — `/openapi`); microservices keep internal contracts. Changing a microservice never changes the SPA while the BFF endpoint is stable.
...
## Conventions
- Built on Hono (`/framework-hono`), non-VPC by default, Pattern B; **routes at root**; OpenAPI generated from them (`/openapi`) = the contract API GW imports (`/api-gateway`).
- **No auth code in the BFF** — claims come from the API GW Cognito authorizer (`/api-gateway`, `/cognito`); the SPA holds the JWT via the Cognito SDK (`/authentication`).
