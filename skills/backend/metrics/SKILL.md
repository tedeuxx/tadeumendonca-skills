...
```
- **Flush once per invocation** — call `metrics.publishStoredMetrics()` in a `finally`, wired in the Hono handler/middleware (`/framework-hono`); optionally `metrics.captureColdStartMetric()`.
- The EMF lands in the BFF log group `/aws/lambda/<project>-bff-${env}`; CloudWatch extracts metrics under namespace `<project>/${env}` (`/cloudwatch`).

## Conventions
- **No `cloudwatch:PutMetricData`** — EMF metrics are extracted from logs, so the exec role needs no metrics IAM action (basic logs perms suffice). See `/iam`.
- **Low cardinality** — dimensions limited to `action_type` / `environment` / `service`. Never `user_id` or id-bearing paths.
- Suggested metrics: request count + latency, cache hit/miss (`/redis-cache`), DynamoDB query duration, handler errors.
- `og-edge` (Lambda@Edge) emits no metrics (edge constraints).
- Powertools owns Logger / Metrics / Tracer uniformly (`/logging`, `/tracing`).
