...
component library** (no shadcn/ui, no Cloudscape). It pairs with `/framework-react` (the impl home)
and `/storybook` (where components are documented). **Project-specific identity** — the actual
...
- Components are **hand-built primitives** in `src/components/`, documented in `/storybook`.
...
- **UX states** — explicit loading / empty / error primitives (`/ux-states`), not a library spinner/status widget.
