---
description: Instrument product analytics in a React SPA with Google Analytics 4 — a page_view on every route change, because an SPA never reloads, plus custom events for key actions and no PII. Use when measuring what users actually do, adding an event, or keeping non-production traffic out of the property. Not for performance and error telemetry (see cloudwatch-rum).
---

Frontend analytics with GA4 (concept).

Context: $ARGUMENTS

Conceptual skill — the analytics contract. The gtag snippet lives in `/framework-react`.

Product analytics via **Google Analytics (GA4)** — page views + events. SPA-aware: send a **page_view on every route change** (SPAs don't reload). Complements RUM (`/cloudwatch-rum`), which is performance/errors.

## Contract
- Load GA4 once with the measurement id (from SSM — `/environment-config`).
- `page_view` on each route change; custom `event`s for key actions (e.g. `article_open`, `subscribe`).
- **Production only** (or a separate property per env) so staging doesn't pollute data.

## Conventions
- **No PII** in events. Measurement id from SSM, never hardcoded. Respect Do-Not-Track / consent if added.

## Pros & cons
**Pros**
- Free, ubiquitous web analytics; SPA `page_view` per route + custom events.
- No backend work — client-side tag.
**Cons**
- Client-side, so ad-blockers/consent reduce data.
- Privacy/consent handling required; Google dependency.
