---
description: Set client-side SEO in a React SPA with no SSR — per-route title, description, canonical, Open Graph and JSON-LD, plus a build-generated sitemap.xml and a static robots.txt. Use when a page needs correct metadata, adding structured data, or keeping browser metadata identical to what the edge serves crawlers. Not for the crawler render path itself (see prerender).
---

Frontend SEO (concept, no SSR).

Context: $ARGUMENTS

Conceptual skill. The react-helmet-async snippet + sitemap script live in `/framework-react`.

Two layers: **per-route meta** (title, description, canonical, OG, JSON-LD) so Google's JS rendering + browser tabs are correct, and build-time **`sitemap.xml` / `robots.txt`**. The crawler heavy-lifting is at the edge (dynamic rendering — `/og-edge-handler`, `/prerender`); this is the client baseline. The app stays CSR — **no SSR**.

## Contract
- Each page declares its meta: `<title>`, description (≤160), canonical (absolute, prod domain), OG/Twitter, and **JSON-LD** (`Person` for CV, `BlogPosting` for articles).
- `robots.txt` (static) + a build step that generates `sitemap.xml` from the articles list (`/github-actions`).
- Keep client meta **consistent with the edge prerender** output (same title/description) — not cloaking.

## Conventions
- No SSR framework — the architecture is CSR + edge dynamic rendering. Canonical = production domain; one JSON-LD per page.

## Pros & cons
**Pros**
- Per-route meta + JSON-LD structured data without SSR; pairs with the edge prerender for bots.
**Cons**
- CSR meta is not seen by every crawler (the reason edge prerender exists).
- Two SEO paths to keep aligned.
