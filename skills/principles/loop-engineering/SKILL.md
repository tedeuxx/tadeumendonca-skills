...
- **Proposing a change to the MACHINERY — dispatch `harness-lead` before implementing it.** This is the persona of this discipline: added 2026-08-04 as the owner's pair in their *harness-engineer* role, on hooks, settings and permissions, agent briefs, skills, commands, the plugin and MCP. It returns the scenarios a proposal does not cover, each with **how to check it or labelled a hypothesis**, and its standing question is ADR-0008's — *which layer can actually carry this control, and can that layer hold it?* ~~It is **advisory and pre-implementation**: it gates nothing~~ — **struck by [ADR-0015](../../../docs/adr/0015-harness-lead-implements-the-harness-it-reviews.md)**, which reverses the pre-implementation-only framing: the owner may now have it implement the harness changes it stress-tests, under the same "cannot merge" mitigation `tech-lead` already carries. Check `agents/harness-lead.md:4`'s current `tools:` line before assuming either state — this note holds whether or not the `Write, Edit` grant has landed yet. What that ADR leaves unchanged: it **still reviews no merge request, still merges nothing** (rule 7b's catch-all) **and still opens no Issue itself** (rule 5d's catch-all — a proposal Issue is filed by the orchestrator on its naming, not by it), so it never sits in the intake chain the leads run (`/dev-loop`). What changed is that it is no longer true that it "gates nothing": a harness diff now requires its verdict marker on the PR before `quality-assurance` may classify that diff safe (ADR-0015, Corollary 2) — a gate that runs through the marker `quality-assurance` reads, never through a merge or an MR review of its own.

  **Why the two lines above are not the same dispatch, since both sound like "validate a change".** `tech-lead` and `quality-assurance` judge a change *to the product*, against records and against a Definition of Done. `harness-lead` judges a change *to the loop that judges the product*, and the failure it exists to catch is not a wrong decision but an **inert** one — a deny on a tool no hook observes, a glob that does not reach the second repo of a two-repo workspace, a persona merged while a third still runs the brief that predates the merge. Four of those were found in a single day, all by accident, all after implementation.
