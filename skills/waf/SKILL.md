...
Two WebACLs via **`cloudposse/waf/aws ~> 1.0`** (`/terraform`), split by ownership:
...
- Logs go to an `aws-waf-logs-<project>-${env}` group (mandated prefix — `/cloudwatch`); WAF holds no at-rest data of its own. TLS is terminated at CloudFront / API GW, which enforce TLS 1.2+ (`/kms`).
