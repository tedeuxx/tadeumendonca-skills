...
Serves the HTML the Lambda@Edge returns to bots (see `/og-edge-handler`). Two endpoints, same data source (DynamoDB), **no React on the server** — this is content templating, not SSR.
...
- HTML must mirror what the SPA renders (same content) — not cloaking. Keep `og-meta` and `prerender` titles/descriptions identical to the client `/seo` output.
- Public routes (no JWT); cached at the edge (`max-age=300`). Hit/miss can feed `/metrics`.
- The OG PNG itself comes from `/og-image-generator`; prerender only references its URL.

## Decision & trade-off
- **The bot API mirrors what the SPA renders from the SAME data store — it is content templating, not SSR and not cloaking.** `og-meta` (JSON for social) + `prerender` (full HTML + JSON-LD for crawlers) read the domain repositories directly; no React on the server. *Trade-off:* a **second render path** that must stay in lockstep with the client SEO output (`/seo`) — keep titles/descriptions/JSON-LD identical or it drifts toward cloaking.
