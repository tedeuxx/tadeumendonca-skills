...
## Rule
Verify **both axes** for every new resource before merge. `checkov` enforces many of these (`/terraform`).

## In transit (TLS)
| Service | How | Skill |
|---|---|---|
| CloudFront | `minimum_protocol_version = "TLSv1.2_2021"`, `viewer_protocol_policy = redirect-to-https` | `/cloudfront` |
| API GW (REST) | HTTPS-only custom domain (ACM); no HTTP | `/api-gateway` |
| Cognito hosted UI | HTTPS (ACM, us-east-1) | `/cognito` |
| DynamoDB | reached over HTTPS via the Gateway VPC endpoint (AWS SDK TLS by default) | `/dynamodb` |
| Redis | `transit_encryption_enabled = true` + AUTH token | `/elasticache` |
| S3 | reached only via CloudFront OAC over HTTPS; bucket policy denies `aws:SecureTransport = false` | `/s3` |

## At rest
| Service | How | Skill |
|---|---|---|
| DynamoDB | encrypted at rest by default (AWS-managed `aws/dynamodb` key) | `/dynamodb` |
| Redis | `at_rest_encryption_enabled = true` | `/elasticache` |
| S3 artifacts | **SSE-KMS** (`aws/s3` key + bucket keys) | `/s3` |
| S3 fed + og-images | **SSE-S3 (AES256)** — CloudFront OAC can't decrypt the `aws/s3` KMS key (see note) | `/s3` |
| Secrets Manager | KMS (`aws/secretsmanager`) by default | `/secrets-manager` |
| SNS topic + SQS DLQ | `kms_master_key_id` (`aws/sns`, `aws/sqs`) | `/sns` |
| CloudWatch Logs | encrypted (CMK when required) | `/cloudwatch` |
| Lambda env vars | AWS-managed Lambda key (`kms_key_arn` for a CMK) | `/lambda` |
...
- **Use a customer-managed key (CMK)** only when you need one of: cross-account/grant control, custom rotation schedule, key-usage auditing (CloudTrail), or one shared key across resources. Provision via `terraform-aws-modules/kms/aws` (`/terraform`).
- **Rotation:** any CMK sets `enable_key_rotation = true`.
- **Least privilege:** a CMK key policy grants `kms:Decrypt`/`Encrypt`/`GenerateDataKey` only to the roles that need it (e.g. the BFF exec role for Secrets/Redis/DynamoDB data keys — `/iam`) — never `kms:*` to `*`.
...
- A service needing `kms:Decrypt` adds it to its exec-role statements **only when using a CMK** (`/iam`); with AWS-managed keys no explicit grant is needed.
- Tag CMKs via provider `default_tags` (`/terraform`).
## Decision & trade-off
