...
Define or review the audit trail in the BFF.

Context: $ARGUMENTS

Conceptual skill — *what* an audit record is and how the trail works. The framework-specific wiring (the Hono middleware that captures and writes it) lives in `/framework-hono`.
...
The "who" comes from the **JWT claims** the API GW Cognito authorizer already validated and injected into the request (`sub`, `cognito:groups`) — read per request, **zero lookup, no session cache**. This is why the BFF stays stateless and Redis is cache-only (`/redis-cache`).
...
```jsonc
{
  ...
  "action_type": "posts_create",          // the route's declared action (/action-types)
  "user": {
...
## Wiring
The Hono middleware `audit(action)` that runs after the handler and `Put`s this item is defined in `/framework-hono`; the action constants in `/action-types`; the table access/client in `/dynamodb`.
