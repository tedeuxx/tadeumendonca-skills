...
iteration would never fire. This is #103's arithmetic arriving one layer down, and `/autonomy-on` has
already struck the same sentence for the same reason.
...
1. A `loop` Issue joins the **active** iteration at filing (#338).
2. Every eligible `loop` item is composed **ahead of every `product` item** at planning (#339).

**So fifteen findings do not queue behind the product work — they displace it, by rule, in the very next
iteration.** That is not a side effect anyone has to notice; it is the standing composition rule doing
what it was made to do, applied to a source of items that did not exist when it was made.
