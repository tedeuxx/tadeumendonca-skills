...
The SPA (`<apex-domain>`) and the BFF API (`api.<apex-domain>`) are **different origins**, so every BFF call is cross-origin. **The API Gateway owns CORS** — expressed in the **OpenAPI body** (a REST API has no native `cors_configuration`), so it is reproducible and survives `put-rest-api` (`/infrastructure/api-gateway`):
- `OPTIONS` (MOCK) + gateway responses set `Access-Control-Allow-Origin` = the **exact** SPA origin per env (`https://<apex-domain>`, `https://staging.<apex-domain>`) — **never `*`** (we send `Authorization`); `allow_methods = [GET, POST, PUT, DELETE, OPTIONS]`; `allow_headers = [authorization, content-type]`; `expose_headers` only if the SPA must read a custom response header (e.g. a pagination cursor); `max_age = 300`.
...
- **The BFF (Hono) does NOT set CORS** — the gateway answers the `OPTIONS` preflight; CORS lives only in the OpenAPI body (one owner, never hand-configured in the console — `put-rest-api` overwrite would wipe it) (`/backend/bff`).
