...
IaC has its own gate (`checkov`, in `/infrastructure/terraform`) and is not covered here.
...
|---|---|---|
| browser | **E2E journeys** — the critical paths (home, feed, authenticated entry), driven through a real browser | `/frontend/playwright` |
| browser | **component library** — interaction and visual tests, where a component library is used | `/frontend/storybook` |
| browser | unit/component runner + coverage thresholds | `/frontend/framework-react` |
| server | **API/contract run** — the collection executed against a deployed API: 401 without a bearer token, 200 with it, schema checks | `/backend/postman` |
| server | unit/integration runner + coverage thresholds | `/backend/framework-hono` |
...
- **SAST + quality gate** — the **SonarCloud** Quality Gate blocks merge and imports the unit-coverage
  lcov, so SAST, coverage and smells are judged together (`/workflow/sonarcloud`).
- **Secrets** — secret scanning, and nothing sensitive committed. The stacks differ only in where a
  ...
  (`/backend/secrets-management`), and in a browser bundle **there is no such place at all** — anything
...
- Where the gates sit in the pipeline: `/workflow/github-actions`. What "done" means, above any
  particular gate: `/principles/verification-and-gates`.
