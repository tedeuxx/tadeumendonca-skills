...
**Decision & trade-off:** a *light* gate (significance-triggered), not a *strong* one (ADR for every change). Trade-off: a light gate can miss a decision that only looks routine — and the mitigation **moved rather than halved**, which is worth stating because the sentence here said otherwise until 2026-08-03. ADR-0002 amendment #7 retired `plan-reviewer`; **amendment #8 put the design-time half back in the intake chain** — `tech-lead` applies the significance test *before* the build and writes the record, `quality-assurance` verifies on the MR that a significance-crossing change references one. Two reviewers, at the same two moments as before, under different names. A strong gate would never miss one but taxes every trivial change and trains people to write empty ADRs; the light gate keeps ADRs meaningful.
