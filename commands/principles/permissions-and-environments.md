...
Permissions encode where the irreversible line falls, so they follow the repo's loop model (`/principles/dev-loop`). Determine it before writing a single allow/deny entry — an allowlist built for the wrong model pre-authorizes branches that do not exist and leaves the real boundary open.
