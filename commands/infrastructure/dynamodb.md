...
plus their `/index/*`** (`/infrastructure/iam`) — never `dynamodb:*` on `*`, and **never `Scan`**. So
...
```hcl
# infrastructure writes the names (/infrastructure/ssm) — non-sensitive, names only
...
renamed in Terraform reaches the code without a code change (`/backend/environment-config`).
...
above. Read-heavy reads go through cache-aside (`/backend/redis-cache`).
...
return { items: res.Items ?? [], next_cursor };   // snake_case; client side = /frontend/pagination
```
...
— traffic stays on the AWS backbone, **off the NAT path** (`/infrastructure/vpc` declares the `dynamodb`
Gateway endpoint alongside `s3`). HTTPS/TLS in transit by default either way (`/infrastructure/kms`).
...
- **Lambda@Edge cannot reach DynamoDB at low latency** — prerender/OG data is served by the regional
  API, not the edge (`/backend/og-edge-handler`).
- A single-partition GSI (feed `gsi_pk="POST"`) is fine at this scale; shard the PK if it ever gets hot.
- Tags via provider `default_tags` (`/infrastructure/terraform`); encryption stance
  `/infrastructure/kms` (AWS-managed `aws/dynamodb` key, no CMK).
...
- **Per-entity tables + GSIs over single-table design.** One table per domain aggregate, with a GSI per
  ...
  transactional read in one call, so aggregation happens in the API's shaping layer (`/backend/bff`).
...
- **Cursor pagination via the opaque base64 `LastEvaluatedKey`** — the continuation token IS the cursor,
  ...
  forward/continuation paging only (the contract the client consumes — `/frontend/pagination`).
