---
description: Provision and instrument real-user monitoring end to end — the Terraform app monitor, a Cognito guest identity pool whose role can only put RUM events, the browser client reporting web vitals, JS errors and HTTP latency, and the sampling that bounds the bill. Use when field performance is unknown or a browser error is invisible server-side. Not for product analytics (see frontend/analytics) or server-side logs, alarms and traces (see infrastructure/cloudwatch, infrastructure/cloudwatch-xray).
---
...
(`/frontend/analytics` is the latter, and they answer different questions).
...
`rum:PutRumEvents` on this monitor's ARN and nothing else (`/infrastructure/iam`).
...
build-time configuration (`/backend/environment-config`) rather than as secrets. The client snippet
itself lives with the framework (`/frontend/framework-react`).
...
- Encrypted log group and tagged like everything else (`/infrastructure/kms`,
  `/infrastructure/terraform`).
- Pairs with `/infrastructure/cloudwatch-xray` for the server half of the trace, and with
  `/infrastructure/cloudwatch` for server-side logs and alarms.
