...
scheduled out. The operative instruction lives in `commands/new-issue.md`'s *Open it* step — the file that
performs the filing — and this is the rule it executes, stated once here.

**Scope is `loop` and only `loop`.** A `product` or `content` Issue filed mid-iteration takes no milestone
and is composed into an iteration at planning, which is where `/autonomy-on`'s count of `ready` items
carrying no milestone finds it. Widening the filing rule to every type would move planning into the
capture command, which is a different decision and not this one.

**Which repo's iteration: the one the Issue is filed in.** Milestones are a per-repo namespace and there
is no cross-repo iteration object, so *"the current iteration"* is two objects and the filing repo picks
between them. Nothing else resolves it, and a rule that left it unstated would be unexecutable in a
two-repo workspace.

**The premise this negates, and it is the reason this sub-section exists rather than one line in a
command file.** `loop` is the class *generated by working*, so a rule sending it to the current iteration
makes the active pool grow while it is drained. That was the exact sentence `/autonomy-on` reinstated its
terminal condition on. **The owner reopened it rather than narrowing this rule** — see
*What exhaustion means now* below and `/autonomy-on`'s *The pool grows while it drains*, where the
replacement terminal condition lives.
