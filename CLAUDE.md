...
(`ls commands/` → `autonomy-off.md autonomy-on.md blueprint.md new-issue.md retrospective.md`, re-run
...
| **the pair that argues with it** — same tier, and the roster's first true pair | `content-reviewer` (#317) — reads a draft against `published-voice`, the same skill `content-writer` drafted against, for **at most two rounds**; may block only where it can **quote a clause** of that skill, everything else labelled advisory-and-droppable. Terminal on the first round with no citable finding, or on the second round, whichever comes first. Contained by rule 5e like the other two, and its rounds land in a tracked file (`docs/content-review/<slug>.md`) rather than a comment | **reason #1 of the four — disagreement is wanted**, and it is the first persona added on that reason since the four-reason rule replaced the one-line one. The owner's ask: raise the bar of a draft *before* it reaches him. It is not a second gatekeeper — it runs pre-merge on the draft, `quality-assurance` runs post-build on the diff — and it does not judge truth, which stays `product-lead`'s blocking veto at the merge gate. **`product-lead` left the drafting flow on the owner's decision (#317); only the craft opinion left, its veto and its `content` intake did not** |
...
| `/command-hygiene` | Where scratch files go, one atomic Bash call, the `gh --repo` flag position, `--body-file` always — ~~preloaded by all 7 personas~~ **preloaded by 7 of the 8 (#375): every persona that writes a file or runs a shell command.** `scrum-master` holds neither `Write` nor `Bash`, so the rule has no subject there and preloading it would be bytes with nothing to govern |
