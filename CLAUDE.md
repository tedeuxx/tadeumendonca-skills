...
The **agentic dev-loop** (methodology ADRs `docs/adr/`, design in `docs/proposals/agentic-dev-loop.md`): a team of per-task subagents in `agents/`, materialized lazily. Built so far — the two review gates plus the ADR author: **`plan-reviewer`** (design-time — reviews a plan/spec against the principles + the ADR library for drift, flags ADR-needs; the evolution of the former `principles-guide`), **`critical-reviewer`** (code-time — reviews an MR against the Definition of Done, ADR-0003; approves+merges the safe class, escalates the boundary, ADR-0004), and **`adr-author`** (records a significant decision in MADR — picks library + next number, drafts at `proposed`, updates the index; never merges, per `/workflow/adr`). Together these close the design→record→verify triangle. The rest of the roster (frontend-react, iac-terraform-aws, planner, qa-e2e, sonar-remediator, …) is defined in the proposal; a project enables the subset its blast-radius justifies.
