...
# The per-skill catalogue left CLAUDE.md on 2026-09-22 (the file crossed the harness's
# 150,000-character limit). The row arm below follows it. It is deliberately NOT repointed at
# README.md, which carries a per-skill table too: that one is GENERATED by
# hooks/scripts/skills-table.py, so asserting against it would be a green that cannot redden
# for the reason this arm exists - an author adding a skill and forgetting the row.
CMDREF="$ROOT/docs/command-reference.md"
...
# --- skills, in total ---------------------------------------------------------------------------
# The root-level commands (autonomy.md) are counted SEPARATELY from the library, because that is
# how both documents present them: "<N> skills + autonomy".
#
...
# --- the CATALOGUE skill table (docs/command-reference.md) -------------------------------------------------------------------
#
...
#
# THE TABLE MOVED OUT OF `CLAUDE.md` ON 2026-09-22 and this arm followed it. The property is
# unchanged; only the file is. What DID change is that `CLAUDE.md` no longer carries a row per
# skill at all, so the total asserted there is now a POINTER'S figure rather than a table's
# header - it can still go stale, and this arm still pins it, but it no longer has rows under it
...
# and now covers the whole library at once: every skill in the tree must have a row in the
# catalogue's table. That is the half that was catching things.
expect_in "$README" "$total skills" "library total (README)"
expect_in "$CLAUDE" "$total skills" "library total (CLAUDE)"

missing_rows=""
# Existence first, and for the reason this file already books one block further down: without it a
# moved or deleted catalogue makes every `grep` below fail, which the missing-rows branch reads as a
# CLEAN run and reports as a green. A pass for an unexamined reason, in the suite written to remove
# exactly that.
if [ ! -f "$CMDREF" ]; then
  bad "docs/command-reference.md skill table — the catalogue does not exist, so the row assertion below
      is checking nothing. CLAUDE.md points at it; either the pointer is stale or the file was lost."
else
while IFS= read -r d; do
  [ -z "$d" ] && continue
  stem="$(basename "$d")"
  grep -qF "| \`/$stem\` |" "$CMDREF" && continue
  missing_rows="$missing_rows $stem"
done <<< "$SKILL_DIRS"
if [ -z "$missing_rows" ]; then
  ok "docs/command-reference.md skill table — a row for all $total skills"
else
  bad "docs/command-reference.md skill table — these skills have no row:$missing_rows; a skill is published and unlisted"
fi
fi

expect_in "$README" "$total skills + autonomy" "library total"
...
# sequence there is still available. Adding a skill reddens `<N> skills + autonomy`, one edit fixes
...
#     figure counts the LIBRARY only ("<N> skills + autonomy"), so the two owner-typed commands have
...
#
# ONE THING IT NOW COVERS THAT NOTHING DID: deleting `commands/autonomy.md` used to remove the file
...
# THE ARM IS NO LONGER `/blueprint`-ONLY (#368), AND THE GENERALISATION IS THE FINDING RATHER THAN A
# TIDY-UP. `/autonomy` became a mode-dispatching command with exactly this shape — a dispatch table, an
# `argument-hint` a typist reads, `# Mode:` sections a model dispatches on, and a bare invocation that
# does nothing — inheriting `/blueprint`'s two properties on purpose. A second command with the same
# failure mode and no arm is the second-order effect a rename slice produces and nobody sees: the
# protection looks like it covers the pattern and covers one file.
#
# THE SET IS DECLARED, NOT DERIVED, and that is deliberate. Deriving it (say, every command carrying a
# `# Mode:` heading) would make a command that LOSES its dispatch table silently leave the set, which is
# the exact drift this block exists to catch — a check whose subject is chosen by the thing it checks.
# Adding a third mode-dispatching command is one word here, in the same commit, and forgetting fails
# CLOSED at the vacuity guard below.
MODE_CMDS="blueprint autonomy"

for MODE_CMD_STEM in $MODE_CMDS; do
BP_CMD="$ROOT/commands/$MODE_CMD_STEM.md"
...
if [ ! -r "$BP_CMD" ]; then
  bp_mode_problems="$bp_mode_problems
    commands/$MODE_CMD_STEM.md is not readable"
fi
...
if [ "$bp_modes_n" -lt 2 ]; then
  bad "$MODE_CMD_STEM modes — the dispatch table parsed $bp_modes_n mode(s) and every one of these
      commands has two named ones plus the bare invocation. Either the table moved out from under
      '## The three modes' or its row shape changed, and every assertion here is now vacuous."
elif [ -n "$bp_mode_problems" ]; then
  bad "$MODE_CMD_STEM modes — the dispatch surface disagrees with itself:$bp_mode_problems
  ...
  ok "$MODE_CMD_STEM modes — the $bp_modes_n named mode(s) agree across the argument-hint, the dispatch table and the '# Mode:' sections, and the bare invocation is declared in both"
fi
done

# RESTORE `BP_CMD`, AND THIS LINE IS LOAD-BEARING RATHER THAN TIDINESS. Every arm below this point —
# the triage classes, the adoption rules, the document contract, the tipo->enforcement mapping — reads
# `$BP_CMD` and is about `/blueprint` specifically. Making the block above a loop left the variable
# holding whichever command was last in `MODE_CMDS`, and four arms below went red against a file that
# is not their subject. Found by running the suite, not by reading the diff.
...
AUTON="$ROOT/commands/autonomy.md"
limb2_missing=""
for limb2_needle in \
...
if [ -z "$limb2_missing" ]; then
  ok "PR-link rule — commands/autonomy.md carries the second limb AND names the detector's blindness to it"
else
  bad "PR-link rule — commands/autonomy.md no longer states the rule the owner wrote:$limb2_missing
...
# its authority the two surfaces the same MR had just reversed), `commands/autonomy.md` (the OTHER
...
if [ -n "$lane_gen_missing" ]; then
  ...
  grep -qF -- '**`ready` means the description is closed by whoever closes it on that lane — and on `loop` it is the' "$AUTON" \
    || lane_gen_missing="$lane_gen_missing
    missing: commands/autonomy.md's \`ready\` sentence is no longer lane-scoped"
...
ITER_CMD="$ROOT/commands/autonomy.md"
...
iter_cmd_missing=""
if [ ! -r "$ITER_CMD" ]; then
  bad "iteration axis — commands/autonomy.md is not readable; the executed pool and terminal
  ...
  if [ -n "$iter_cmd_missing" ]; then
    bad "iteration axis — commands/autonomy.md no longer executes the iteration-scoped drain:$iter_cmd_missing
    ...
    ok "iteration axis — commands/autonomy.md scopes the pool to the active iteration, counts the unassigned, and stops on exhaustion"
  fi
fi
...
snap_cmd_missing=""
if [ ! -r "$ITER_CMD" ]; then
  bad "entry snapshot — commands/autonomy.md is not readable; the terminal set cannot be checked."
else
  ...
  if [ -n "$snap_cmd_missing" ]; then
    bad "entry snapshot — commands/autonomy.md no longer terminates against a set fixed at entry:$snap_cmd_missing
    ...
    ok "entry snapshot — commands/autonomy.md terminates against the entry snapshot, keyed on numbers, re-taken per invocation"
  fi
fi
...
LOOPFIRST_CMD="$ROOT/commands/autonomy.md"
...
if [ ! -r "$LOOPFIRST_CMD" ]; then
  bad "loop-first — commands/autonomy.md is not readable; the pointer from the file the loop
  ...
  if [ -n "$loopfirst_cmd_missing" ]; then
    bad "loop-first — commands/autonomy.md no longer routes ordering correctly:$loopfirst_cmd_missing
    ...
    ok "loop-first — commands/autonomy.md points at the composition rule and the wrong citation stays struck"
  fi
fi
...
LOOPBATCH_AUTONOMY="$ROOT/commands/autonomy.md"
...
if [ ! -r "$LOOPBATCH_AUTONOMY" ] || [ ! -r "$LOOPBATCH_NEWISSUE" ]; then
  bad "loop-batch — commands/autonomy.md or commands/new-issue.md is not readable; the pointers from
  ...
  do
    ...
    missing from commands/autonomy.md: \"$loopbatch_autonomy_needle\""
  done
...
if [ ! -r "$LOOPBATCH_SKILL" ]; then
  ...
  if [ -n "$loopbatch_durable_missing" ]; then
    bad "loop-batch — the layer analysis fell back to its dated measurement alone:$loopbatch_durable_missing
      ...
      wrap matches nothing and a null result reads as an absence — the defect commands/autonomy.md
...
EST_CMD="$ROOT/commands/autonomy.md"
...
est_cmd_missing=""
if [ ! -r "$EST_CMD" ]; then
  bad "estimation carrier — commands/autonomy.md is not readable; the preflight cannot be checked."
else
  ...
  if [ -n "$est_cmd_missing" ]; then
    bad "estimation carrier — commands/autonomy.md no longer gates ENTRY on the pendency set:$est_cmd_missing
      ...
      A NOTE FROM #393, BECAUSE IT IS A KNOWN TENSION AND NOT AN OVERSIGHT. This arm requires N
      pendencies to reach the owner as N surfacings, which is the shape /sprint-planning was corrected
      FOR — it stopped at item 1 of 15. The preflight was NOT changed with it: it runs BEFORE a loop is
      running, so by the escalation standard's own precondition («se nao tem loop nao é hitl») its
      surfacings are not escalations and that standard does not reach them. Left as it is deliberately;
      whether it should change is the owner's call and is not this slice's."
  else
...
RITE_DRAIN="$ROOT/commands/autonomy.md"
...
if [ ! -r "$RITE_DRAIN" ]; then
  bad "retrospective rite — commands/autonomy.md is not readable; the pointer from the file that
...
if [ ! -r "$RITE_PRELOAD" ]; then
  ...
  if [ -n "$rite_preload_missing" ]; then
    bad "retrospective rite — the universal preload is out of step with the rite:$rite_preload_missing
      ...
      preloads, so a plural read as satisfied here is read as satisfied everywhere. THAT NEEDLE CARRIES
      ITS OWN '~~' FENCES SINCE #379-r2, AND THE REASON IS THIS SLICE'S OWN DEFECT: it read
      'Read *\"the closing ceremonies\"* anywhere in this loop as' — the bare phrase — which matched
      LIVE text at the merge base and matches only STRUCK text at head, because #379 struck the
      sentence and moved the two needles below it to the plural while leaving this one behind. Measured
      by an independent lens rather than found by reading: restoring the false claim to LIVE in the
      preload, everything else at head, ran '193 passed, 0 failed' — identical to control. The arm was
      green over the preload asserting the opposite of the truth, next to the truth. With the fences in
      the needle it asserts THE SENTENCE IS STRUCK, so un-striking it is what reddens.
...
# ══════════════════════════════════════════════════════════════════════════════════════════════════
# THE ESCALATION STANDARD — ITS FIVE CLAUSES, AND THE PRECONDITION THAT COMES FIRST (#393).
#
# WHY THIS IS A GATE. The owner asked to standardise ONE act — «eu apenas queria padronizar a escalacao
# do loop» — and the standard was inflated twice while it was being written, both times in the same
# direction: every question to him read as an escalation, and the four-option contract got applied to
# conversations it was never about. His correction is a PRECONDITION, «se nao tem loop nao é hitl», and
# a precondition is exactly the clause a later edit drops as throat-clearing, because the form beneath
# it reads complete without it.
#
# THE ARMS ARE TWO AND THEY ARE DIFFERENT IN KIND:
#   1  CONTENT — the five clauses, the precondition ahead of the form, the axes defined literally, the
#      failure test, and the honest enforcement statement. Needled because each is the owner's own
#      wording and a paraphrase satisfies a looser check.
#   2  NEGATIVE — the portable half must NOT state reversibility as the escalation rule. It is the
#      imported blueprint's rule, it is not his, and it is available from a real source, so the risk is
#      RE-ADOPTION rather than invention. An absence is the only form that catches a rule arriving from
#      outside.
#
# WHAT WAS DELIBERATELY NOT BUILT, and the reason belongs here rather than in a commit message. A
# per-rite `## AFK or HITL` declaration arm was written and REMOVED when the ask narrowed: the two
# closing rites are not redesigned by this slice, so an arm requiring them to declare would have gated
# scope the owner cut. It is recorded because the arm existed, passed its mutations, and was dropped on
# scope rather than on evidence — a later reader finding this absence should know it was decided.
#
# WHAT THESE CANNOT DO, and it is most of the standard: NOTHING OBSERVES AN ESCALATION. No permission
# layer sees a message to a human, and no hook sees a turn. Clauses 2, 3 and 4 — that a subagent
# raised it, that it rose through the protocol, that it was a genuine trade — are unreachable by any
# layer here, and the files say so themselves. These arms assert the rules are WRITTEN.
AH_PORTABLE="$ROOT/skills/engineering-standards/SKILL.md"
AH_PRELOAD="$ROOT/skills/agents-configuration/SKILL.md"

# ── 1 · the five clauses, the precondition first, and the honest enforcement statement ────────────
ah_content_problems=""
if [ ! -r "$AH_PORTABLE" ]; then
  ah_content_problems="$ah_content_problems
    skills/engineering-standards/SKILL.md is not readable, so the standard itself was not checked."
else
  for ah_needle in \
    ...
    '**cost** | **tokens.**' \
    'anything that MOVES SCOPE is a *potential* escalation' \
    '### The failure test — a question with no options is not an escalation' \
    '**A bare question is offloading the analysis.**' \
...
if [ -r "$AH_PRELOAD" ]; then
  for ah_pneedle in \
    ...
    '**No AFK/HITL contract table is written into this harness.**'
  do
  ...
  ah_content_problems="$ah_content_problems
    skills/agents-configuration/SKILL.md is not readable, so the LOCAL half — who composes the options,
    what is instrumented, and what does not exist — was not checked."
fi
...
if [ -n "$ah_content_problems" ]; then
  bad "escalation standard — a load-bearing clause is gone:$ah_content_problems
      Each is the owner's own wording and a paraphrase would satisfy a looser needle:
        PRECONDITION   — «se nao tem loop nao é hitl». It is FIRST because it is the clause a later
                         edit drops: the form beneath it reads complete without it, and a reader who
                         meets the form alone stamps every question to him as an escalation. That is
                         not hypothetical — it happened twice while this standard was being written.
        FIVE CLAUSES   — a loop running · a dispatched subagent · rising through the protocol · a
        / ALL FIVE       trade · the form. ALL FIVE is the conjunction, and without it four-of-five
                         passes as an escalation while being a design question or a status report.
        ...
        SCOPE TEST     — the operational trigger, and POTENTIAL is load-bearing: escalate every scope
                         touch and the brevity buys nothing, escalate none and he finds out after.
        FAILURE TEST   — a question with no options is offloading the analysis. This catches the
                         deferential shape — asking what he wants instead of composing what the loop
                         already worked out — which is why 'always carries the options' is needled in
                         the contract AND as its own test.
        NO THRESHOLD   — the calibration is his, from metrics and worklog over real iterations. A
                         ...
                         if someone had.
        ENFORCES       — the honest split, stated IN the file rather than only in a commit: two
                         clauses partly mechanizable, three not at all. Without it a reader takes the
                         standard for a control, which is the failure this whole block is about.
        WORKLOG /      — the LOCAL half: the worklog he named does not exist in this tree, and no
        NO TABLE         AFK/HITL contract table is written here because that is HIS design work. A
        ...
        LEADS          — scrum-master NAMES which leads a scope escalation needs and cannot dispatch
                         them (tools: []), and a disagreement between them IS the trade and rises as
                         the options. Without both clauses this reads as the product ceremony
                         returning through a side door."
else
  ok "escalation standard — the portable half carries the precondition ahead of the form, all five clauses with ALL-FIVE stated, the axes defined literally, the moves-scope test with its 'potential' qualifier, the failure test, no threshold, and its own honest enforcement split; the preload carries the local half, the missing worklog, the absent contract table and the leads-are-named-not-dispatched rule (text only; nothing observes an escalation)"
fi

# ── 2 · the portable half must NOT state reversibility as the escalation rule ─────────────────────
#
...
# word where the file is REJECTING it — it keys on the divergence sentence, not on the term.
if [ -r "$AH_PORTABLE" ]; then
  if grep -qF -- '**This is NOT *reversibility, not seniority*' "$AH_PORTABLE"; then
    ok "escalation standard — the portable half records the divergence from 'reversibility, not seniority' and names it as an imported blueprint's rule rather than the owner's (text only; nothing stops a proposal citing it)"
  else
    bad "escalation standard — skills/engineering-standards/SKILL.md no longer records that the trade
      test DIVERGES from 'reversibility, not seniority'.
      ...
      is trivially reversible and trades scope for time. Without the recorded divergence, the next
