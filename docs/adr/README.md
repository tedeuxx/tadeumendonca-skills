...
| `harness-blueprint` | [0021](./0021-the-blueprint-registry-is-authored-generation-verifies-it.md) | How this harness **describes itself to a reader who does not run it** — the obligation registry and its behaviour-keyed rows, the immutable ids, what an export of it may claim, and what the format deliberately refuses to key on. | The obligations themselves. Every row in the registry *points at* a decision that lives in one of the capabilities above, and describing a control is not deciding it. |
...
absent an `agents-lead` verdict marker. All three were examined for relocation and **none of them
moves**:
...
- **Record 0015's Corollary 2 is verification
  ...
  *`agents-lead` implements the harness it reviews (absorbed 2026-08-20, record 0015)* section.
...
| 0015 | `agents-lead` implements the harness it reviews — unscoped `Write, Edit` mitigated purely by *cannot merge*, and six corollaries: no new hook layer, the harness-diff criterion that makes such a diff boundary class absent a verdict marker, the durable verdict marker against a commit SHA, **owner-only `loop`-typed `ready`**, harness proposals as real `loop` Issues, and the two brief bugs that travel with the grant | [0002](./0002-roster-and-dev-loop.md) — section *`agents-lead` implements the harness it reviews (absorbed 2026-08-20, record 0015)*. Absorbed under [ADR-0020](./0020-an-adr-earns-its-place-by-explaining-the-current-codebase.md)'s fourth disposition on [#283](https://github.com/tedeuxx/tadeumendonca-skills/issues/283), slice S4 |
...
| 0017 | ADR authorship is split by domain — `tech-lead` writes product/system records, `agents-lead` writes loop/machinery records | [0020](./0020-an-adr-earns-its-place-by-explaining-the-current-codebase.md) — section *ADR authorship is split by domain — `tech-lead` writes product/system records, `agents-lead` writes loop/machinery records (absorbed 2026-08-20, record 0017)*. Absorbed under [ADR-0020](./0020-an-adr-earns-its-place-by-explaining-the-current-codebase.md)'s fourth disposition on [#283](https://github.com/tedeuxx/tadeumendonca-skills/issues/283), slice S6 |
