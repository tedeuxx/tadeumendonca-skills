...
**And this ADR cannot apply that variable, because the owner has not adopted it.** [ADR-0004](./0004-autonomy-and-permission-model.md)'s 2026-08-02 amendment raises decidability and records it *"Not adopted: the owner has not ruled on it."* Its **decided** rule is still reversibility — *"if it can be fixed in the next commit, a hook costs more than it returns"*, with *"what 'finished' means"* as the example of something skill-shaped. **Under the decided rule a revertible merge does not earn a hook, and this ADR loses.** It escapes only through the variable that is not adopted, and the first version of this record papered over that by claiming the question was already settled elsewhere. It is not settled anywhere.

**So this record is an adoption request, and the ratification is where it is granted or refused.** Ratifying ADR-0007 adopts **mechanical decidability** as the variable that puts a rule in the hook row, alongside or in place of reversibility. That is the actual decision on the table, and it is larger than one hook — which is a reason to say it out loud rather than to bury it.

What the variable buys, if adopted: *do two markers parse, carrying a literal from each persona's canonical set, at this `headRefOid`* is decidable by a script with no judgement. *Is the change right* is not, and never becomes so. Before [ADR-0006](./0006-a-verdict-owed-to-another-persona-is-an-artifact.md) the precondition was prose in an orchestrator's context and there was nothing mechanical to test at all; the marker is what made half of it decidable. **That is the change of fact this record turns on**, and it is one day old.

**Two axes, named because conflating them is what made the first version incoherent.** *Decidability* decides whether a rule earns the hook row. *Blast radius* decides how that hook should fail when it cannot reach an answer. The fail-open section below argues from revertibility and is not in tension with the paragraph above it — it is answering the second question, not re-opening the first.
