...
4. **The same hook, but `ask` instead of `deny`** — `permission-guard.sh` already has an `ask()` and rule 5c uses it for exactly this shape. **Partially adopted**, and the split is the decision: `ask` **only** where the check could not run *and* no outsider could have caused that; `deny` everywhere else. *Why not for everything:* the loop's value is that in-pattern work merges without waking the owner, and a hook that asks on **every** merge converts the safe class into a prompt.
...
- [ADR-0003](./0003-mr-definition-of-done.md) — the Definition of Done this gate reviews against. **Not the source of the author-and-association idiom**, and this hook checks no ratification rule.
