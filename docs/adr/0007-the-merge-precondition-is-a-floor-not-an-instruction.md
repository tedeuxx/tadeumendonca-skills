...
> **The hook fetches a bounded, fixed-size page and never paginates.** Latency then depends on the page, not on how many comments a stranger posted, so it is no longer a quantity anyone outside can choose. Markers absent from that window fall to the rule above and **deny**.
...
- **The harness cap sits ABOVE this hook, and nothing in the hook can reach it.** `hooks.json` kills a hook at its timeout, and a killed hook emits no decision, which the harness treats as **allow** — `wip-guard.sh` records that measured fact. Bounding the read removes the attacker's ability to *select* that path, and it does not remove the path: a genuinely slow API on a genuinely slow day still ends in a merge nobody checked. Not fixable from inside the hook, and not claimed to be.
