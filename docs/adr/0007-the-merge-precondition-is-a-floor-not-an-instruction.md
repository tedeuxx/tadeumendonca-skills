...
**The unresolved case ASKS. It does not allow, and the reserve option above is where that comes from.** Three versions of this record said *allow*, and each time a gatekeeper found a party outside the trust boundary who could reach it — first by posting an unparseable comment, then by making the read truncate. The last one is what settled it: **comment volume is a single attacker-controlled quantity, and `allow` on `timeout` routes it to the opposite outcome from `deny` on truncation.** A stranger simply picks the amount. The chain: `hooks.json` caps a hook at 10s, and a hook that emits no decision is treated as allow — the **measured** instance of that is `wip-guard.sh`'s record of `permission-guard.sh` with `jq` off PATH, and the killed-by-cap case is the same shape inferred from it rather than separately measured, which is worth the distinction in a record about claims that read stronger than they are. Enough comments, and the merge proceeds with **no marker verified**.
...
**`ask` is necessary and not sufficient, and the part it cannot reach is why the READ MUST BE BOUNDED.** `hooks.json` caps a hook at 10s, and a hook the harness kills **emits nothing at all** — which the harness treats as allow. That is above the hook: no wording in the table, `ask` included, can make a killed process say anything. So the attacker-selectable path has to be closed where it is created, at the read.

> **The hook fetches a bounded, fixed-size page, never paginates, and holds its own deadline strictly below the harness cap.** The page bounds how many comments are read; the deadline bounds the wait regardless. Markers absent from that window fall to the rule above and **deny**.
