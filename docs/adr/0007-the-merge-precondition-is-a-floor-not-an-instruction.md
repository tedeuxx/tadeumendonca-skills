...
**And it already qualifies under the rule ADR-0004 has decided, so nothing here needs a new variable.** That record's line as decided: *"If the act **cannot be undone**, it needs a hook — `terraform destroy`, a force-push, `rm -rf`, a secret write, **a push to the trunk** all escape git, and no later commit undoes them."* A `gh pr merge` **is** that push to the trunk — `permission-guard.sh`'s own rule 7b says so in its deny message, *"merging a PR is the deploy"*. The merge is in the decided set already.

**Two earlier versions of this paragraph got that wrong in opposite directions, and both are worth recording.** The first argued from irreversibility without noticing the same document called the deploy revertible, so it argued against its own existence. The second over-corrected: it declared the decided rule fatal, reached for ADR-0004's *"Not adopted"* decidability variable, and framed this record as an **adoption request** — asking the owner to settle a question larger than one hook. Both were wrong for the same reason: neither read the clause of the decided rule that is directly on point.

**What was actually revertible is the site, and that is a different object from the act.** A wrong page is fixed by the next merge. The *merge itself* cannot be un-pushed — the deploy fires, the release is cut, an OG scraper pins the card it first fetched. ADR-0004 encodes precisely that distinction and this record simply failed to apply it.

**Decidability still matters, but as a feasibility test rather than a licence.** *Do two markers parse, carrying a literal from each persona's canonical set, at this `headRefOid`* is decidable by a script with no judgement; *is the change right* is not, and never becomes so. That is what makes the hook **buildable** and what bounds it to half the precondition. Before [ADR-0006](./0006-a-verdict-owed-to-another-persona-is-an-artifact.md) there was nothing mechanical to test at all — the marker is what made this half decidable, **and that change of fact is one day old.**

So two axes, kept apart because conflating them is what produced both bad versions: **irreversibility** puts the rule in the hook row (ADR-0004, already decided) · **decidability** says how much of it a hook can carry · **blast radius** decides how that hook fails when it cannot reach an answer, which is the fail-open section below.
...
Chosen: **the hook on `gh pr merge`**, because the precondition is mechanically decidable and the party it binds is the one reading it.
...
- **The stranger class this ADR introduces has its own residual, and it needs a line in the ledger rather than only a paragraph in the Decision.** Because the repository is public, the author filter is what stands between a drive-by comment and a merge — and the filter it borrows from ADR-0003 admits `COLLABORATOR`. **So a collaborator who is not a gatekeeper passes it.** That is narrower than "anyone with a GitHub account" by an enormous margin and it is not nothing; on a repo with one collaborator it is currently vacuous, which is exactly the kind of fact that stops being true without anyone revisiting the record. Named so the next person to add a collaborator finds it.
...
- [ADR-0003](./0003-mr-definition-of-done.md) — the DoD the gate reviews against, and the source of the author + association idiom this hook borrows. **Its ratification rule stays prose and this hook does not check it** — see the scope paragraph in Decision outcome. *(This line previously said the hook "would also check" the ratification. That was true of the wider scope and survived the narrowing — the residue `security` predicted the narrowing would leave, found in the section that was not re-read.)*
