...
### 0049 · which decisions are the human's, and the form an escalation takes

- **tipo:** knowledge
- **carrier:** `skills/engineering-standards/SKILL.md`
- **descrição:** Two rules for a loop that runs unattended: a decision is the human's when it trades **time** (work hours plus wait hours), **cost** (tokens) or **scope** against each other for that work item — anything moving scope is a *candidate* — and an escalation is a tweet at most, at most four direct options, terse on first contact, always carrying the options.
- **propósito:** A loop that runs unattended has to answer *which decisions does it take and which does it hand up* before it can answer anything else, and the two rules most often reached for are both wrong here. **Seniority** sorts by who feels senior. **Reversibility** — the governing rule of at least one widely-circulated loop blueprint — sorts precisely backwards: it makes the irreversible-but-already-decided act attended, and the cheap-but-genuinely-contested one unattended. And the form is not presentation: an escalation interrupts a person doing something else, so **the cost is the context they must rebuild, not the decision**. A rite that spends fifteen of their turns has moved unattended work onto them whatever it claims about itself.
- **o que faz:** States the triangle scoped per work item, with all three axes defined literally so the wrong instrument is not reached for. Gives the operational test — **anything that moves scope is a *potential* escalation** — and four worked cases including the one that gets missed, a finding that would widen an item beyond what it promised. Fixes the activation's form and adds the failure test that catches the deferential shape: **a question with no options is offloading the analysis**, so if it cannot be reduced to four direct options it is not ready to be escalated and the loop owes more work first.
- **o que não faz:** **It authors no threshold** — no number, no multiplier, no trigger condition for how far a spend runs before the trade is worth escalating; that calibration comes from metrics and worklog over real iterations, and inventing one would be a decision nobody made in a record that reads as if someone had. It does not settle *how* a loop detects that an item's cost or time has gone wrong: that question is open and no mechanism answers it. It is **not** the permission floor's irreversibility test, which is untouched and asks a different question — what may never execute without a human, rather than whose decision a choice is. And **nothing enforces any of it**: no permission layer observes a message to a human, and no artifact records how many turns an activation cost unless the loop writes one itself.
