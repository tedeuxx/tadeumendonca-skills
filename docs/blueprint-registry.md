...
- **o que faz:** Reads the tool call's command string before it runs and returns a `deny` decision for the acts on the floor — infrastructure mutation, `rm -rf`, hard reset, GitHub secret writes, repository deletion, `--dangerously-skip-permissions`, and — since the S3-revert put them back, so the list is not read as the floor's whole membership — force-push, AWS secret writes and `gh repo archive`/`rename`. ~~secret writes, force-push and hard reset~~ — **struck 2026-09-05 (#383 S3): the floor is IRREPARABILITY, and three acts that reverse were moved off it.** ~~It now returns `ask` rather than `deny` for force-push (the reflog and the remote's unreachable objects both hold the old tip), AWS secret writes (version stages, a 30-day recovery window, SSM parameter history) and `gh repo archive`/`rename` (both undo, and the OIDC trusts pin the immutable id rather than the name).~~ **Struck 2026-09-05 (#383 S3-revert, the owner's ruling, the same day): all three return `deny` again.** The reparability argument that moved them is untouched and still true — what failed is the **rung**. A hook `ask` is answered automatically in this harness's default working mode (measured in the owner's own session: the guard returned `ask` and the act executed with no prompt), so `deny → ask` was not a downgrade here but a removal. Each act is still split from an irreparable sibling — `git reset --hard`, `gh secret set`, `gh repo delete` respectively — and since the revert that split survives only where it still can: the **same** verdict, a **different** message, exactly as `descrição:` above says. ~~**An unanswerable `ask` was measured to fail CLOSED**, in a headless main session and in a dispatched subagent alike, so this is a loosening only where a human is present to answer.~~ **Struck with it — the measurement is right and the conclusion drawn from it was wrong.** An unanswerable `ask` does fail closed in a headless session and in a dispatched subagent; neither is the case that decides this element. That case is the **interactive main session under auto mode**, where the `ask` is answered without a human ever seeing it, which is why the downgrade was withdrawn rather than kept behind a caveat. It unwraps the common indirections (`bash -c`, quoting forms) so a wrapped payload is matched rather than hidden. ~~It is deliberately **branch-agnostic**: no `git branch` call, no environment-name matching, so one hook is correct under both loop models.~~ **Struck 2026-09-02 (#383).** The trunk-push rule resolves the checked-out branch (`git -C "$dir" symbolic-ref --short HEAD`) and denies when it is `main`/`master`; "no `git branch` call" was literally true and the property was false. What is genuinely absent is **environment-name matching**, and that alone is what makes one hook correct under both loop models.
...
- **o que faz:** Denies `$(...)`/backticks and `VAR=x cmd` prefixes at the leading position or within recognized simple compositions (#438 — the measured runtime probes stopped both forms). The added env-prefix check first recognizes the entire original string as plain words, blanks, simple subshells and separators; complex shell text falls outside it and receives no decision from that extension. This is a bounded matcher, not a shell parser or a universal runtime-subset proof. It also denies `>` / `>>` redirection that would create a file, returning the replacement route in the denial text (`git -C`, `npm --prefix`, the `Write` tool). The redirect rule was made mechanical only after guidance alone was measured to keep failing.
...
- **descrição:** A `SessionStart` hook that reports the version of the build the session is **actually running**, whether it is behind the source, and which other registered projects are pinned to a different build.
...
- **o que faz:** Derives its own version from **the artifact it was invoked as** — the manifest that ships beside the running copy, reached from the invocation path — rather than from a registry describing what ought to be installed, and injects it as context, so every claim made about a hook in that session can be read against the build that was actually running. A second arm reads the install registry and names the **other** projects pinned to a different build, because the project that is most behind is by definition the one nobody opens.
- **o que não faz:** It never blocks, and it reaches the **parent** context only — a subagent never sees a `SessionStart` notice, which is precisely the actor most likely to be reasoning about a hook. And it does not update anything: knowing the build is behind is not being on it. **Nor does it reason about which registry record belongs to this session** — that question is what it stopped asking: the running copy answers *what am I* directly, and every heuristic that re-derived it (path matching, record precedence, unknown scopes) failed silently and in the direction of comfort. The cross-project arm needs a JSON reader, and where there is none it says so rather than reporting nothing.
...
### 0047 · the absence of a summons is a state a harness can observe, and the presence of one is not the same question

- **tipo:** record
- **carrier:** `hooks/scripts/owed-pr-link-detect.sh`
- **descrição:** A `Stop` hook that reads the turn's own assistant prose for the ABSENCE of a review-artifact link, when the artifact's own state says the remaining act is the human's.
- **propósito:** A loop whose gate executes almost everything itself has exactly one class of moment where the human must act, and that class has no announcement. The artifact says so — it is open, its checks concluded, and the gate's own verdict at that revision names the human — but nothing reads the artifact against the turn that just ended, so the loop can finish, fall silent, and look identical to a loop still working. **The sibling that watches for a link handed over prematurely cannot see this**: its predicate is a link's presence, and every silent turn is invisible to it by construction.
- **o que faz:** Resolves the artifact from the branch rather than from a link — there is no link, which is the point — asks the same three mechanical questions the sibling asks (open, every check concluded and successful, and a verdict at that revision naming the human as the remaining actor), then reads the assistant's own prose for the turn that just ended and reports, once per (artifact, revision) per session, when none of it carried the artifact's address.
- **o que não faz:** **Detection, never prevention** — it fires after the turn already reached the human, so there is nothing left to add. **And it does not address the incident it was built under, which its own header says before it says anything else:** the motivating complaint was a link the human had received twice and could not find, buried mid-message; an absence detector is silent on exactly that, and shipping it as the answer would make its silence read as *communication was clear*. Nor does it judge placement: the two measured positions were an announcement near the top and the real ask far down, so a position threshold would pass the non-ask and flag the ask. It is blind to the shorthand the rule recommends, for the sibling's reason, and here that blindness costs a false positive rather than a miss.
- **citação:** > "Position does not separate an announcement from a summons; only intent does, and intent is not in the string."
...
- **o que não faz:** **Nothing fires it, nothing observes it, and nothing bounds it** — the trigger is an instruction in another command file, no hook in this harness reads the queue at all, and the two-finding cap lives in a template because no layer can count findings in prose. The consult set is a **lower bound**, never the set: the dispatch recorder exits silently on about a dozen paths, so a persona that ran and left no trace is indistinguishable from one that never ran. It cannot see a defect that lived **between** two contexts, since every reporter reports only its own. And it finds no defect a reader would meet — that is the sweep half's class, which is `0048` since #379 and was not built when this row was written.
- **citação:** > "Isolated speculation is still speculation."

### 0048 · an iteration closes by looking at the running product, from a surface nobody wrote down

- **tipo:** record
- **carrier:** `commands/sprint-review.md`
- **descrição:** A typed rite the drain runs first of three at iteration close — one dispatch to the only persona holding a browser, which sweeps the live product across a derived surface and writes one observation report per iteration.
- **propósito:** Every gate in a loop of this shape reads a **diff**, and a diff cannot carry the defect a reader actually meets: a banner off-centre, a bar rendering without its link, a layout that reads wrong at a phone width. The obligation is not "hold a demo" — there is no audience and no acceptance moment. It is that a loop whose merges are deploys should look at what it deployed, once per iteration, with somebody who knows what the page was *for*. The failure this closes is specific and was measured before it was designed: three defects of that class shipped with every gate green and were found by the owner on his phone.
- **o que faz:** Derives its targets from the product's own route generator — the same function the sitemap and the prerender consume, so a route that exists is in the set by construction — visits each at a phone and a desktop viewport in every locale, and reads each page's assets **off the page itself** through the network log and the DOM snapshot rather than from any list. Reports two halves separately, mechanical evidence apart from judgement, leads with `routes emitted` and `routes visited`, and writes one tracked file into the consuming repository whose judgement half is an input to the next planning.
- **o que não faz:** **It is not a gate and returns no verdict** — a looker's finding is taste, and a gate with no ruler grades taste, so every finding here is advisory and droppable including the mechanical ones. It **ships no route list**, which is what makes the derivation load-bearing rather than decorative. It **declares itself a lower bound**: the viewport set is enumerated and can go stale, anything behind an interaction is unreachable under a read-only browser grant, and an emulated phone is not the phone the motivating defects were found on. It **mutates nothing**, never authenticates, and opens no work. And **nothing fires it, nothing observes it and nothing bounds it** — no hook in this harness reads the queue, and the report lands in a repository the gate that checks this rite never sees.
- **citação:** > "It is NOT a gate, and it returns NO verdict."

### 0055 · the audience is read once per period, and a rite that could not read says so in a different word

- **tipo:** record
- **carrier:** `commands/funnel-review.md`, `scripts/funnel-review.sh`, `docs/funnel-review/README.md`
- **descrição:** A typed rite at the close of a period that reads the published funnel end to end — reach, click, read, share, contact — against the pieces published in that period, stores what it read, compares it against the prior period, and returns at most two findings. Its collection step runs through a browser the harness does not own, so its report prints one of **two distinct literals**: the surfaces were read, or they were not.
- **propósito:** A loop whose output is published prose has gates that all read a **diff**, and not one of them can see whether anything published was reached, read or acted on. The obligation is not "collect metrics" — it is that the feedback loop be **repeatable**, which the one-off sitting that motivated it was not: it had no questions written down, no store, no baseline and no cap, so its finding could not be re-derived and its numbers could not be compared against anything. The second half of the purpose is the one worth porting: where a rite's data lives behind a login the harness does not hold, **did-not-run and ran-and-found-nothing are the same silence**, and a rite that does not separate them reports a healthy funnel over a dark one.
- **o que faz:** Splits the rite at the exact line the credential question falls on. **Collection** happens wherever an authenticated browser is reachable — on the ruled route, the owner's own already-authenticated session, so nothing is stored and nothing is granted to the harness — and its whole output is a tracked file whose first declaration is whether the surfaces were read at all. **Analysis** needs no browser, no network and no credential: it reads that file, refuses to print any figure that arrived without a sample size, attaches a **surface-aware** ceiling clause to each one — the site's consent ceiling where the site's banner gates the data, and a different clause where the number is a platform's own unstated sampling — finds the prior period by name and reports its absence rather than implying a trend, and caps the findings at a number declared in the source and printed into every report. **It splits a collected line with pathname expansion OFF**, so a figure carrying a `*` or a `?` — ordinary inside an analytics page path or a campaign name — is never replaced by filenames from whatever directory the rite happened to run in; without that, the published number is a function of the working directory, and both paths exit 0. The two states print `FUNNEL-REVIEW-NOT-COLLECTED`, with **no findings section at all**, or `FUNNEL-REVIEW-RAN`, with one that may say it is empty; neither literal is a substring of the other, and one suite runs both branches in the same pass. A fourth line in the cadence record gives the store a clock, which the carrier reads without being edited.
- **o que não faz:** **It is not a gate, not a dashboard, and not a generator of work** — it returns no ruling, renders no chart in place of a finding, and is capped because the sitting that motivated it produced three tracked items in one evening. **Nothing observes the collection step, and no layer could**: the browser route reaches neither registered matcher, so a collection file asserting it read a live console is byte-identical to one somebody invented, and nothing here can tell them apart. Nothing fires the rite — the clock carrier **notices** that the report is stale and cannot dispatch — and nothing pairs a report with the collection it claims to derive from. The cap is held over **landed artifacts only**, so a period nobody reported is invisible to it. It cannot see a reader who declined consent, anything a platform chooses not to report, or **why** any number moved: attribution is not in the data, and inventing it is the failure its own *not a dashboard* clause is aimed at. And **it states no bound for a surface outside its closed vocabulary** — an unrecognised name gets an `UNDECLARED` ceiling rather than a description of one, because the default that described one was asserting a consent claim about surfaces nothing here recognises.
- **citação:** > "A rite that quietly does nothing is indistinguishable from one that ran and found nothing."
...
- **o que não faz:** ~~It does not apply `ready` for a loop-typed Issue: that transition is the owner's alone.~~ **Struck 2026-09-25 (owner ruling on #512): on a loop-typed Issue it applies `ready` only once the owner has said the item is ready, with his words recorded on the Issue — the decision is his, the labelling act is the session's; it never applies it on the lead's closure alone.** And it cannot verify that the leads actually closed the description rather than one nodding it through — the label is auditable and attributable, never proven.
...
### 0046 · an iteration is composed from an assembled pool and confirmed by the human in ONE activation

- **tipo:** routing
- **carrier:** `commands/sprint-planning.md`
- **descrição:** A typed rite that assembles every candidate for the next iteration — the eligible unmilestoned work in both repositories plus the proposals the closing rites produced — has a tool-less profile rank it, composes the iteration from that ranking against rules already ratified, and puts the whole composition to the owner as ONE activation he confirms or changes.
...
### 0053 · a closing rite that is owed is said out loud, on a clock rather than on a boundary

- **tipo:** routing
- **carrier:** `hooks/scripts/cadence-notice.sh`, `docs/loop-cadence.md`
- **descrição:** A session-start notice that reports how long it has been since each closing rite last landed an artifact, against an interval declared in a tracked record. It fires on **elapsed time only** — it makes no tracker call of any kind — and it reports; it decides nothing and blocks nothing.
- **propósito:** The closing rites' only other trigger is a batch boundary, and a batch boundary is not a thing any layer here observes: it is a drain reaching exhaustion of a snapshot, which is an instruction. So the rites have never had a mechanical trigger in any working mode, and nobody was ever told one was owed. **The honest comparison is therefore clock-versus-nothing rather than clock-versus-boundary**, and it is the comparison the owner was given in the option he answered. Measured when this landed, by firing it: two of the three rite artifact roots had never existed at all, and the third read `last written 9d ago`.
- **o que faz:** Reads the commit date of each rite's artifact root — chosen over a stamp the notice writes itself, because nothing can observe a rite *running*, so a self-written stamp would never advance and the notice would fire forever. Compares the newest against a **declared** interval and speaks only when it has elapsed, when nothing has ever landed, or when the interval is **not declared** — in which case it refuses to conclude rather than defaulting, because a default is inference by another route and a carrier that quietly picked a number would report a verdict nobody took. Distinguishes a declared opt-out from an undeclared value, so a carrier the owner turned off does not look like one that is quietly inert. Scopes itself to a tree that declares a cadence, so it says nothing in a consuming repository that never runs the rites. Debounces to one notice per day, into the per-checkout state directory, so the marker can never arrive in a diff.
- **o que não faz:** **It cannot run a rite, refuse a prompt or deny a call**, and every exit path is success — it is the class of element that reports and gates nothing. **It reads no mode, no container, no label and no queue**, deliberately: an empty container is the terminal condition in one working mode and means nothing whatever in the other, so a trigger keyed on emptiness would make the second silently inherit the first's. **An artifact is not a rite** — a rite that ran and wrote nothing is invisible to it, and an unrelated commit touching one of those paths resets its clock. It sees **one root**, so a rite whose report lands in the consuming repository is declared unobservable rather than reported as never-run, and nothing pairs two repositories' records. It reaches the **parent context only**, one session late. And **nothing checks that the observed set is the rite set**: a rite added with no line in the record is invisible to it, and no gate says so.
- **citação:** > "An artifact is not a rite, and that bound rides with every number this carrier prints."

### 0054 · a worktree that is finished is said out loud; nothing deletes it

- **tipo:** routing
- **carrier:** `hooks/scripts/worktree-notice.sh`
- **descrição:** A session-start notice that classifies every linked worktree of the repository the session is in — finished, in progress, locked, prunable, merged-but-not-clean, or carrying commits no ref points at — and names the ones whose removal is lossless. It reports; it removes nothing.
- **propósito:** Opening a worktree is one command and closing one is nobody's step, so the registry only ever grows. Measured in the consuming repository on 2026-09-10: **28 registered, 26 present on disk, 9,565 MB, and all 28 heads already ancestors of `origin/main`** — every one a finished slice. The identical selector returns `1` in the harness repository, so it distinguishes a littered tree from a clean one rather than counting rows. **It is a precondition of admitting parallel development rather than a consequence of it**: the lifecycle is already broken at one-slice-at-a-time, and parallelism multiplies the rate rather than creating the defect.
- **o que faz:** Reads git's own registry — so discovery is authoritative rather than a sibling-tree heuristic, and works from the primary tree, any linked worktree, or any subdirectory of one, under any naming convention. Applies four limbs: not locked, head already on the trunk ref, working tree clean **as `git status` sees it**, and not detached-with-unreferenced-commits. Two of the four are also enforced by git itself, which is the point rather than waste — acting on the notice with the plain command cannot destroy work even if the classification is wrong. Prints the blocking entries for a worktree git will refuse, so the human adjudicates with the evidence. Debounces to one notice per day into the **common** git directory, so the marker cannot arrive in a diff and cannot fire once per worktree.
- **o que não faz:** **It removes nothing, refuses nothing, and never prints `--force`** — `--force` overrides two of the four limbs at once, so offering it would hand the reader the one command that turns safe advice into an unrecoverable act. It **cannot tell a finished worktree from one somebody is about to use**: the four limbs answer *is removing this lossless*, never *does anyone want this*. `git worktree lock` is the declared opt-out for the second question and **0 of 28 worktrees used it**, so a worktree nobody locked is reported as removable however live it is — a residual the lock narrows and does not close, which is precisely why nothing here deletes. It sees **one repository**, is **one session late**, and reaches the **parent context only**. And **nothing obliges anyone to act on it**: by this loop's own test it is not engineered, and the removal it recommends still depends on a human running the command.
- **citação:** > "It never prints --force, and a worktree git refuses is listed with what is blocking it."
...
- **descrição:** The pair to the content builder — at most two rounds, **repairing the draft in place** on two named grounds rather than blocking on it.
...
- **o que faz:** Reads the draft against the shared voice skill for at most two rounds and **edits it on exactly two grounds** — it can **quote a clause** of that skill, or the claim is **false against the source**, with the source the only permitted supply of the replacement. Everything else is labelled advisory-and-droppable and the prose is left alone. It writes its rounds to a tracked file on the same branch, and the corrected piece goes to the held preview so the owner reads the result rather than a verdict about it.
- **o que não faz:** ~~It does not judge **truth** — that stays a blocking veto at the merge gate~~ — **struck 2026-09-03: it holds the copy lens on this lane now, and there is no copy block left at the gate.** What it still cannot do is **verify against the world** (the code, another surface, durability), and a false claim its source cannot settle it **cuts** rather than rewrites, because inventing the replacement is what the ruler forbids. It is not a second gatekeeper: it runs pre-merge on the draft, the gate runs post-build on the diff. Its advisory half is droppable by construction, which is the price of the two-round bound.
...
- **descrição:** The portable half of the former universal preload — two tiers, twelve principles, delivery against hygiene, and the human residual.
...
- **o que faz:** States the mechanical-or-it-is-not-real test and, beside it, the second test that asks whether the control that exists **can fail** — *a check whose positive result is unconditional is not a check*, calibrated by breaking the subject on purpose and never the checker, with the shapes it takes outside a test suite. Then the two tiers (a floor that never bends and a dial calibrated to blast radius), the twelve principles as defaults ~~plus their triggers to deviate~~ — **struck #410: read beside a numbered list that phrasing says *each carries one*, and exactly one of the eleven did. Two of twelve do now (principles 1 and 12), and the four floor principles cannot carry one at all, since a tier that never bends has nothing to state a deviation from** — what counts as delivered against hygiene, what an agent does while blocked on someone it does not control, and what is left to the human.
...
- **tipo:** knowledge
- **carrier:** `skills/engineering-standards/SKILL.md`
- **descrição:** A single standard for ONE act — an escalation rising out of a running loop. Five clauses, all required: a loop is running · a dispatched subagent hits something on an item in it · it rises subagent → main session → human · the trigger is a trade of **time** (work plus wait hours), **cost** (tokens) or **scope** · and the form is a tweet at most with **at most four direct options** ~~, always carried~~. **Amended 2026-09-03: the options are carried on a DECISION and never on an ACTION**, and the unqualified clause struck here is the one that caused the defect — see `0051`. **Amended 2026-09-09 (owner ruling, *«Trocar por issue em andamento»*): clause 1's ANCHOR is a FILED ITEM BEING WORKED and was ~~an iteration in flight~~ — a substitution and not an addition, so which container the item sits in is no longer part of the test.** The re-basing is what keeps the precondition from depending on a configuration axis the standard was never about: where a container is optional, or bounds nothing, or is carried by no open item, the container-anchored clause never holds — so every clause beneath it is unreachable while every surface still reads as though the contract were in force.
- **propósito:** A loop that runs unattended has to answer *which decisions does it take and which does it hand up*, and the two rules most often reached for are both wrong. **Seniority** sorts by who feels senior. **Reversibility** — the governing rule of at least one widely-circulated loop blueprint — sorts backwards: the irreversible-but-already-decided act becomes attended, the cheap-but-genuinely-contested one unattended. And the form is not presentation: an escalation interrupts a person doing something else, so **the cost is the context they must rebuild, not the decision**. **The precondition is the half that gets lost** — without it every question to the human reads as an escalation and the four-option contract is applied to conversations it was never about, which is how this standard was mis-scoped twice while being written.
...
- **o que não faz:** **It authors no threshold** — no number, no multiplier, no trigger for how far a spend runs before the trade is worth escalating; that calibration comes from metrics and worklog over real iterations. It does not settle *how* a loop detects that an item's cost or time has gone wrong: that question is open with no mechanism. It is **not** the permission floor's irreversibility test, which is untouched and asks what may never execute without a human rather than whose decision a choice is. And **it is almost entirely unenforced, which the carrier states itself**: clauses 2, 3 and 4 are not mechanizable at all — nothing records that a subagent's return was an escalation, and no layer sees a message typed to a human. ~~clauses 1 and 5 are partly mechanizable … and every available layer is detection one turn late.~~ **Struck 2026-09-03: the last clause was false about escalations raised THROUGH A TOOL** — a structured picker is a tool call, so a pre-execution layer reads its payload before the human does, and prevention there is genuinely AVAILABLE. ~~One shape of clause 5 is now genuinely prevented (`0051`)~~ — **struck the same day: the control built on that opening was DELETED in the slice that built it, so NOTHING is prevented here.** It refused genuine decisions in a direction the human could never observe, which `0051`'s tombstone states in full. **Available is not the same as worth building**, and the whole of this standard — every clause, including clause 5 — remains exactly as unenforced as this row said before either strike.
...
- **o que faz:** States the fork that makes this lane unlike every other — it is **selected one piece at a time and never drained**, so `ready` here is not a queue and the pool predicate excludes it by construction. Names the human moment at every step rather than describing automation that does not exist. Records the hold's exact mechanism (a shared-fact flag in front matter, a parameter that is presence and never a value, a **divergence** between the public enumeration and the resolution index) and the three alternatives rejected by measurement, so nobody proposes them again. It fixes how the link the owner receives is **formed** — every segment read out of the artifact, the slug from that edition's own front matter, and never a status code as the substitute, since the catch-all rewrite answers `200` for a path that does not exist and a held piece is prerendered in no edition at all. And it puts the flagship trade-off where it cannot be missed: **isolation is not privacy** — a held piece's text ships in the public bundle, and the falsifier for that is published with its mutation.
...
### 0052 · how work FLOWS is configurable, and what may never happen is not

- **tipo:** knowledge
- **carrier:** `CLAUDE.md`, `docs/loop-mode.md`, `commands/autonomy.md`, `docs/prompts/loop-mode-contract.md`
- **descrição:** A contract stating that the loop's working mode is a **named, widely-known agile method** — an enum whose first two members differ on what the iteration container MEANS (a batch commitment against a period label, never present against absent) and on whether the three rites run — and, beside it, the **untouchable list**: the classes of thing a mode may vary and the classes it may never touch. It carries a work-in-progress **slot** with no value, and it is stated twice — once for the context that runs a mode, once as a prompt for a harness on machinery nobody here has measured.
- **propósito:** A loop that makes *how work flows* configurable, without deciding first what a mode may **not** reach, has built a configuration surface that can arrive at the irreversible floor by a route nobody reviewed. The obligation is that **the untouchable list is written before the modes are** — and that it is written as a **measurement over the enforcement layer** rather than as a principle, because a principle cannot be falsified by anyone and a measurement can be re-run by everyone. The second half of the purpose is legibility: a mode named after a method a stranger already knows can be steered by that stranger, which an invented word cannot.
- **o que faz:** Names the two modes and fixes what each one means — container, ceremony set, ordering act — ~~and states plainly that the lighter mode is **specified and not operative**, so the table is not read as a switch already thrown~~ — **struck: the lighter mode is operative.** It carries a **record of which mode is running** — a tracked file with the value, the closed enum, a start date back-dated from an artifact rather than from the day it landed, and **both queue predicates published in full side by side** rather than one plus a description of how to edit it. The record is read by the artifact that executes the queue, **at entry and before the query**, because the two predicates are indistinguishable by their output and a reader that consults the mode afterwards has already lost what it needed; an unrecognised value is **refused by name** rather than defaulted, since defaulting is inference by another route. Publishes the untouchable list as a table of classes, with the load-bearing sentence beside it, and backs it with two calibrated commands over the registered enforcement layer showing that no component reads any object a mode varies. Declares `wip` as a slot and names the boundary with the Issue that owns its value. Prices what breaks first at WIP > 1 — the presence-only reviewer marker, not the head-scoped gate. Lists, per decision, what actually holds it. And records the three rulings it is written against — the cadence carrier is to be built as a notice, the estimate stays in both modes, and the container is demoted rather than removed — noting that **two of the three REMOVED an axis a mode may vary**, which is the shape to want: a mode config gets better by shrinking, because every axis is a place two modes can drift apart with nothing watching.
- **o que não faz:** **It enforces nothing: not one of its decisions is held by a layer that could stop the act.** ~~six of its seven decisions are instructions~~ — struck with the tally it counted, on this registry's own rule that a number beside a table rots separately from it. ~~Nothing reads the mode record~~ — **corrected: the file the loop executes reads it at entry, before the queue query, which is a rule a session performs and not a mechanism; nothing MECHANICAL reads it and no check asserts the record exists or parses.** Nothing checks that the mode was read before a queue query, which is the one place in the design where a wrong guess is silent; nothing detects two roots disagreeing about the mode, because a per-session component sees one root while a mode is a workspace property. It **decides no WIP value and no parallel topology**, it **does not build the cadence carrier** the ruling authorises — a contract says what a mode may vary and a trigger is a mechanism, so the carrier is its own slice, and it became **0053** below — and it **does not decide that carrier's interval**, which was never asked and which 0053 does not decide either. **Its central measurement is dated, not permanent**: it is a fact about today's registered components, and the moment one is taught to read the mode the separation stops being structural. And the untouchable list is a list of **classes**, so it cannot tell a reader whether a specific new rule belongs on it.
- **citação:** > "A mode selects a predicate and a ceremony set. It never selects a permission rule."
