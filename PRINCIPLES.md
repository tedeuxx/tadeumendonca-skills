...
## The loop — two models
The shared spine: `roadmap/PLAN.md → thin slice (WIP=1) → develop locally → validate locally (the repo's regression) + self-verify gates → /code-review → PR`. What differs is only how the change is promoted, and **the heavy gates always sit at the point of no return**:
...
## Full detail
- `/principles/loop-engineering` — **the discipline that names the whole (Loop Engineering / AI-DLC)**: the AI-native loop treated as the engineered artifact — its cadence, its gates-as-a-system, and the harness itself. The other four are its parts.
