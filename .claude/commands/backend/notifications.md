...
The `notifications` module of the BFF — emails registered subscribers (e.g. on a new post) and manages subscriptions. SES is provisioned in `/infrastructure/ses-email`.
...
```
- Lambda role needs `ses:SendEmail` (`/infrastructure/ses-email`); reaches SES via NAT.
...
- **SES sandbox:** new accounts only send to verified addresses — production access is a one-time manual request (`/infrastructure/ses-email`).
